...
#		about.set_logo_icon_name(Gtk.gdk.pixbuf_new_from_file("search.png"))
	...
	def do_stuff(self, widget):
		...
		if parsed[0] == "g":
			...
			url = "https://wiki.xcade.net/index.php?search="
			search = parsed[1]
			webbrowser.open(url + search)
			sys.exit()

		elif parsed[0] == "teg":
			url = 'https://teg.avature.net/#Search/Type: "all", In: "everything"/'
			search = parsed[1]
			webbrowser.open(url + search)
			sys.exit()
		...
		elif parsed[0] != "g" or parsed[0] != "y" or parsed[0] != "w" or parsed[0] != "aw" or parsed[0] != "teg":
